...
public static partial class GameSaveService
{
        ...
        return new GameSaveSnapshot
        {
            ...
            PoliticalCrises = state.PoliticalCrises
                .Select(crisis => new PoliticalCrisisSnapshot
                {
                    ...
                    RelatedCharacterId = crisis.RelatedCharacterId,
    ...
    private static OrderSnapshot CaptureOrder(Order order)
    {
        ...
        return order switch
        {
            ...
            OppositionActionOrder typed => snapshot with
            {
                CountryId = typed.Country.Id,
            ...
            CourtActionOrder typed => snapshot with
            {
                CountryId = typed.Country.Id,
                SubjectCharacterId = typed.Subject.Id,
                CourtAction = typed.ActionType
            },
    ...
    private static SaveOrderKind GetOrderKind(Order order)
    {
        return order switch
        {
            ...
            CourtActionOrder => SaveOrderKind.CourtAction,
            _ => throw new NotSupportedException(
                $"Saving order type {order.GetType().Name} is not supported.")
        };
    }
